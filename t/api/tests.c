/*
 * ProFTPD - mod_explain API testsuite
...
int main(int argc, char *argv[]) {
  ...
  requested = getenv("EXPLAIN_TEST_SUITE");
  if (requested) {
    Suite *suite;
    ...
    if (suite) {
      ...
      fprintf(stderr,
        "No such test suite ('%s') requested via EXPLAIN_TEST_SUITE\n",
        requested);
      return EXIT_FAILURE;
    }
  ...
  requested = getenv("EXPLAIN_TEST_NOFORK");
  if (requested) {
